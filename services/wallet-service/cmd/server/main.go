...
import (
	...
	"github.com/omniful/payment-platform/services/wallet-service/internal/handler"
	"github.com/omniful/payment-platform/services/wallet-service/internal/repository"
	"github.com/omniful/payment-platform/services/wallet-service/internal/service"
...
func main() {
	...
	cfg.HTTPPort = getEnv("HTTP_PORT", "8004")
	...
	uc := usecase.NewWalletUsecase(repo, redisClient)
	h := handler.NewWalletHandler(uc)
	walletConsumer := service.NewWalletConsumer(uc, log)

	startConsumer(ctx, cfg, kafka.TopicWalletCredited, walletConsumer.HandleWalletCredited)
	startConsumer(ctx, cfg, kafka.TopicWalletDebited, walletConsumer.HandleWalletDebited)
	...
	go func() {
		log.Info("wallet-service starting", logger.FieldService(cfg.ServiceName))
